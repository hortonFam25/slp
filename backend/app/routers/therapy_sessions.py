...
async def update_therapy_session_objectives_by_appointment(
    ...
    """Update therapy session objectives with pre-session notes for an appointment"""
    try:
        # Get the therapy session for this appointment
        session = repo.get_session_by_appointment_id(appointment_id)
        
        # If no therapy session exists, create one
        if not session:
            from app.models.appointment import Appointment
            from app.schemas.therapy_session import TherapySessionCreate
            from app.db.database import get_db
            ...
            # Create therapy session for this appointment
            session_data = TherapySessionCreate(
                student_id=appointment.student_id,
                appointment_id=appointment.id,
                session_date=appointment.start_datetime.date(),
        ...
        # Update or create session objectives
        ...
        for obj_data in objectives_data:
            ...
            existing_objective = db.query(SessionObjective).filter(
                SessionObjective.therapy_session_id == session.id,
                SessionObjective.objective_id == obj_data["objective_id"]
            ).first()
