...
router = APIRouter(prefix="/api", tags=["progress-entries"])
...
def get_progress_entries(
    ...
    return repo.get_progress_entries(
...
def get_progress_entry(entry_id: int, db: Session = Depends(get_db)):
...
def get_objective_progress_entries(objective_id: int, db: Session = Depends(get_db)):
...
def create_progress_entry(entry: ObjectiveProgressEntryCreate, db: Session = Depends(get_db)):
...
def get_latest_progress_entry(objective_id: int, db: Session = Depends(get_db)):
